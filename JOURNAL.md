...
- **Thiết kế Database Schema (ERD):** Chốt 5 bảng: `USERS`, `CASES`, `SESSIONS`, `STEP_ATTEMPTS`, `CASE_TAGS`. Xác nhận dùng JSONB cho `pipeline_rubric` và `answer_key` trong bảng `CASES`.
- **Thiết kế Sequence Diagram:** Hoàn thành 2 luồng chính — Session Initialisation (CV Agent chạy async) và Answer Submission & Pipeline Advance (score gate ≥ 0.6 để chuyển bước).
- **Scope Lock — Checkpoint Q1:** Review lại MoSCoW framework, chốt danh sách MUST cho sprint 3 tuần, đóng băng WON'T để tránh scope creep. Xác nhận không có auth trong tuần 1 triển khai.
- **Phân công sprint rõ ràng:** Backend chịu trách nhiệm state machine + 3 endpoint; Frontend bắt đầu wireframe case library + image viewer; ML/AI bắt đầu prompt engineering CV Agent và Socratic Agent.
